...
function getProjectName(projectDir) {
  // Try to extract project name from CLAUDE.md or directory name
  const claudeMd = path.join(PROJECTS_DIR, projectDir, 'CLAUDE.md');
  if (fs.existsSync(claudeMd)) {
    const content = fs.readFileSync(claudeMd, 'utf-8');
    const match = content.match(/^#\s+(.+)$/m);
    if (match) return match[1].trim();
  }
  // Use directory name, clean up the E-- prefix pattern
