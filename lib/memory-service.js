...
// Dynamically resolve the current user's .claude/projects directory
const PROJECTS_DIR = process.env.PROJECTS_DIR ||
  path.join(os.homedir(), '.claude', 'projects');

// Generate Claude Code project hash from a file path
// E.g. "E:\claude\my-project" → "e--claude-my-project"
function pathToProjectHash(absPath) {
  return absPath
    .replace(/^([A-Za-z]):\\/i, '$1--')   // Windows drive "C:\" → "C--"
    .replace(/[\\/]/g, '-')                 // path separators → "-"
    .replace(/[^a-zA-Z0-9-]/g, '-')        // non-ASCII/special chars → "-"
    .toLowerCase();
}

// Default project for creating new memories (derived from current working directory)
const DEFAULT_PROJECT = process.env.DEFAULT_PROJECT ||
  pathToProjectHash(process.cwd());
...
function getProjectPath(projectDir) {
  ...
  // Fallback: clean up the hash for display (strip drive prefix like "E--")
  return projectDir.replace(/^[a-z]--/i, '').replace(/-+/g, ' ').trim() || projectDir;
}
  ...
  for (const proj of projectDirs) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const id = `${proj.projectDir}::${path.basename(file, '.md')}`;
        let mtime = '', birthtime = '';
        try { const st = fs.statSync(filePath); mtime = st.mtime.toISOString(); birthtime = st.birthtime.toISOString(); } catch(e) {}
        memories.push({
          id,
